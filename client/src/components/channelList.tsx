...
export default function ChannelList({
  ...
  return (
    <>
      {/* Text channels */}
      <div style={{ padding: "0.75rem 1rem 0.3rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: "0.62rem", fontFamily: "'Share Tech Mono', monospace", letterSpacing: "0.08em", color: theme.textDim }}>
          // TEXT CHANNELS
        </span>
        <button
          style={{ background: "none", border: "none", cursor: "pointer", fontSize: "1rem", lineHeight: 1, padding: "0 0.2rem", color: theme.textDim }}
      ...
      {textChannels.map(ch => (
        ...
        >
          ...
          {ch.name === activeChannel && (
            <div style={{ width: "5px", height: "5px", borderRadius: "50%", flexShrink: 0, background: theme.primary, boxShadow: `0 0 6px ${theme.primary}` }} />
          )}
          {ch.created_by !== null && (
            <span
