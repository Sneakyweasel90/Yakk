...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post(`${config.HTTP}/api/auth/login`, {
        username,
        password,
      });
      await login(data);
    ...
    <div style={{ ...styles.container, background: theme.background }}>
      ...
      <div style={{
        ...
        <div style={styles.logoWrap}>
          <h1 style={{ ...styles.logo, color: theme.primary, textShadow: `0 0 20px ${theme.primaryDim}` }}>
            YAKK
          </h1>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  grid: { position: "absolute", inset: 0, backgroundSize: "40px 40px" },
  glow: {
    position: "absolute", top: "30%", left: "50%", transform: "translate(-50%, -50%)",
    width: "600px", height: "400px", pointerEvents: "none",
  },
  box: { position: "relative", zIndex: 1, width: "360px", padding: "2.5rem", borderRadius: "2px" },
